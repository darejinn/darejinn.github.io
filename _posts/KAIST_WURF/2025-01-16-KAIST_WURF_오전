...
# Background : 의생명 영상화 기술
## 영상화 대상의 재탄생
영상 기법(하드웨어)의 발전을 기다리지 말고, 샘플을 발전시키자
=> 영상화를 위한 조직공학 기술을 발전시키자
<br>
...
> 일반적으로는 더 강력한 현미경(또는 영상 장비)을 개발함으로써 해상도를 높이려 노력하지만, **‘영상화 기법(하드웨어)의 한계를 기다리는 대신, 영상화 대상(조직·샘플)을 개선’**하는 방법을 모색하는 것도 중요한 돌파구가 될 수 있다. 영상 기법(하드웨어)의 발전을 기다리지 말고, 잘 관찰할 수 있는 샘플을 만드는 것은 어떨까?
...
## MAP 기술(2세대 초고해상도 현미경) : Magnified analysis of proteome technology
...
- 1. 빛이 산란되어서(랜덤하게 꺾여서) 깊이/투과도가 깊어질수록 관찰이 어려움. :  => 산란시키는 물질을 제거(지질 추출)
- 2. 빛이 굴절되어서(빛이 미세하게 꺾여서) 초점/선명도가 낮아짐. => 굴절률 균일화 (투명화 용액)
- 3. 조직 시료에서 'contrast'를 만들어야 관찰을 할 수 있다.
...
> '조직 탄성화'가 해결할 수 있지 않을까? : 질기고 잡아당겨지는 entagled hydrogel

## ELAST(조직 탄성화 기반 혁신 기술)
...
2. 항체 시약을 조직 속에 전달하는 데에 걸리는 시간(샘플이 두꺼울수록 오래 걸린다)
  ...
  - 탄성화 조직을 일시적으로 얇게 만들어서, 초고속으로 염색할 수 있다! (두께 조절을 통한 초고속 항체 염색)
  - 늘리고 고정하고(stretch) 물을 condense시켜 가로세로의 길이는 유지된 채 두께를 혁신적으로 얇게 만든다.
<br>
# 3. 공간오믹스(spatial-omics)의 신기술 개발 : 뇌 신경망과, 생체분자와, 생체 기능(세포의 성격, 기능과 같이 구조 이상의 정보)을 보고 싶다
...
## connectome(완전한 신경망 정보) : 뇌가 가지는 궁극의 정보는 망(네트워크)이다
...
대부분의 task를 사람보다 컴퓨터가 더 잘하는 시대에, 우리는 어떤 존재가 되어야 하는지 고민하였을 때, 있는 데이터를 학습하여 성능을 내는 인공지능과 다른 우리의 가장 큰 특징은 없던 생각을 해내는 능력이라고 생각한다. 그리고 연구는 내가 가진 창발적 생각을 비약, 회유, 속임 없이 세상에 설득하는 가장 정직한 방법이라고 생각한다.
